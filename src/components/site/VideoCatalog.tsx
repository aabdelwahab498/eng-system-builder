...
/**
 * Book-style video catalog: an open two-page spread with a real page-turn
 * animation, thumbnail strip, keyboard arrows and a full-screen lightbox.
 * Mirrors ImageCatalog but renders <video> on the media page.
 */
        ...
        {items.map((item, i) => (
          ...
          >
            ...
            <span className="absolute inset-0 flex items-center justify-center bg-black/30">
              <Play className="size-4 text-white/90" fill="currentColor" aria-hidden />
            </span>
          </button>
        ))}
      </div>
...
function VideoPage({ item }: { item: VideoCatalogItem }) {
  return (
